...
def film_and_category(category_df: pdf.DataFrame, film_category_df: pdf.DataFrame, 
    ...
    joined_df = category_df.join(film_category_df, category_df.category_id == film_category_df.category_id,'left') \
...
def ten_actors_most_rented_count(actor_df: pdf.DataFrame, film_actor_df: pdf.DataFrame, 
    ...
    joined_df = actor_df.join(film_actor_df, actor_df.actor_id == film_actor_df.actor_id)\
    ...
    result_df = joined_df.groupBy(actor_df.first_name, actor_df.last_name, actor_df.actor_id)\
...
def category_most_spend_money(category_df: pdf.DataFrame, film_category_df: pdf.DataFrame, 
    ...
    result_df = joined_df.groupBy(category_df.category_id, category_df.name)\
...
def film_not_in_inventory(film_df: pdf.DataFrame, inventory_df: pdf.DataFrame) -> pdf.DataFrame:
    ...
    result_df = film_df.join(inventory_df, film_df.film_id == inventory_df.film_id,'left_anti')
...
def top_3_actor_chikdren_category(category_df: pdf.DataFrame, film_category_df: pdf.DataFrame, 
    ...
    actor_film_count_df = children_df.groupBy(actor_df.actor_id) \
    ...
    result_df = top_ranked_df.join(actor_df, top_ranked_df.actor_id == actor_df.actor_id) \
...
def city_with_active_inactive_customers(city_df: pdf.DataFrame, address_df: pdf.DataFrame, 
    ...
    result_df = joined_df.select(city_df.city,col("active_customer"),
                                 col("inactive_customer"))\
